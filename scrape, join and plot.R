...
clean_rank <- champs_rank %>% select(year_id, rank, fran_id, av_point, result)
...
champ_plot <- ggplot(clean_rank %>% group_by(year_id), aes(year_id, av_point, color = result)) +
  ...
  geom_label_repel(data = filter(clean_rank, result == "champs" & rank == 1), 
            aes(year_id, av_point, size = 12, label = paste("",fran_id,",",year_id,"")), show.legend = FALSE) 
  
champ_plot + scale_x_continuous(breaks = seq(min(1940), max(2015), by = 5)) +
  ...
  scale_color_discrete(name = "NBA Champions",
                    labels = c("Champions", "Not Champions", "NA"))
